...
export const CreateEventView = () => {
  ...
  const {eventFormValues, images} = useAppSelector(
    state => state.createEventFormSlice,
  );
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        const interval = setInterval(async () => {
          try {
            const {status: checkStatus} = await checkAssetsUploadStatus(
              uploadId,
            );
    ...
    <ScrollView>
      <View style={{paddingHorizontal: 16, gap: 12, paddingBottom: bottom}}>
        ...
        <PrimaryFormInput
          ...
          inputStyle="Primary"
          onChangeText={value => handleChangeFormValue('title', value)}
        />
        <PrimaryFormInput
