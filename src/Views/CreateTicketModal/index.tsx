...
const CreateTicketModal = ({ route }: ICreateTicketModal) => {
    ...
    const [createTicketErrors, setCreateTicketErrors] = useState<ICreateTicketErrors>({
        description: "",
        name: "",
        amount: "",
        price: ""
        ...
        <KeyboardAvoidingView behavior='padding' style={{ flex: 1 }}>
            <TouchableOpacity onPress={() => navigation.goBack()} activeOpacity={1} style={{ flex: 1, backgroundColor: "#00000067", alignItems: "stretch", justifyContent: "center", paddingHorizontal: 16 }}>
                <StyledTicketForm onPress={() => Keyboard.dismiss()} activeOpacity={1}>
                    {/* <H5>Create ticket</H5> */}
                    <StyledInputsContent>
                        <PrimaryFormInput value={localTicketValue.name} onChangeText={(value) => handleChangeLocalTicketValue("name", value)} label={createTicketErrors.name || 'Ticket name'} placeholder='Awesome ticket' inputStyle='Primary' />
                        <PrimaryFormInput style={{ minHeight: 80 }} multiline value={localTicketValue.description} onChangeText={(value) => handleChangeLocalTicketValue("description", value)} label={createTicketErrors.description || 'Ticket description'} placeholder='Something about ticket' inputStyle='Primary' />
                        <View style={{ gap: 8 }}>
                            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                                <Span>{createTicketErrors.amount || 'Ticket amount'}</Span>
                                <TouchableOpacity onPress={changeAmountValue}>
                                    <Span textcolor='Primary'>{localTicketValue.amount === -1 ? "Enter limit" : "Make unlimited"}</Span>
                                </TouchableOpacity>
                            </View>
                            <PrimaryFormInput value={localTicketValue.amount === -1 ? "Unlimited" : localTicketValue.amount.toString()} onChangeText={(value) => setFiltersStringValues("amount", value)} keyboardType='numeric' placeholder='Ex. 10' inputStyle='Primary' />
                        </View>
                        <View style={{ gap: 8 }}>
                            <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                                <Span>{createTicketErrors.price || 'Ticket price'}</Span>
                                <TouchableOpacity onPress={changePriceValue}>
                                    <Span textcolor='Primary'>{localTicketValue.price ? "Make free" : "Enter price"}</Span>
                                </TouchableOpacity>
                            </View>
                            <PrimaryFormInput value={localTicketValue.price ? localTicketValue.price.toString() : "Free"} onChangeText={(value) => setFiltersStringValues("price", value)} keyboardType='numeric' placeholder='Price in $' inputStyle='Primary' />
                        </View>
                    </StyledInputsContent>
                    {ticketIndex === undefined ?
                        <PrimaryButton onPress={handleChangeTicketsValue} title={t('createTicket')} /> :
                        <View style={{ flexDirection: "row", gap: 8 }}>
                            <PrimaryButton style={{ flex: 1 }} onPress={handleChangeTicketsValue} title={t('changeTicket')} />
                            <PrimaryButton btnType='Error' onPress={() => ConfirmAlert(handleDeleteTicket, "Do you really want to delete this ticket?")} title={t("deleteTicket")} />
                        </View>
                    }
                </StyledTicketForm>
            </TouchableOpacity>
